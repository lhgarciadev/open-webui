# Etapa 1.1 - Ejecutar Integración de Imágenes

## Objetivo

Implementar servicio unificado de imágenes con Unsplash (principal) y Pexels (backup).
...
```
Implementa el servicio de imágenes para presentaciones:
...
2. IMAGE SERVICE (image_service.py):
   - Función async search_image(query, size="regular") con fallback:
     a. Buscar en Unsplash
     b. Si falla o no hay resultados → buscar en Pexels
     c. Si ambos fallan → retornar None
   - Función download_image(url) con cache LRU (50 imágenes max)
   - Función extract_keywords(text, max_words=3) para búsqueda
   - Retornar: {url, photographer_name, source: "unsplash"|"pexels"}

3. INTEGRACIÓN EN presentations.py:
   ...
   - Agregar atribución pequeña (Pt 8) debajo de imagen

4. PLACEHOLDER (cuando no hay imagen):
   - Rectángulo con gradiente brand-100 → brand-200
   - Emoji 📷 centrado (opcional)
...
- [ ] Revisar código actual de presentations.py
