<script>
	...
	import BudgetManager from './Users/BudgetManager.svelte';
	...
	$: {
		...
		selectedTab = ['overview', 'groups', 'budgets'].includes(tabFromPath) ? tabFromPath : 'overview';
	}
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-4 overflow-hidden">
	<div
		id="users-tabs-container"
		class="mx-[16px] lg:mx-0 lg:px-[16px] flex flex-row overflow-x-auto gap-2.5 max-w-full lg:gap-1 lg:flex-col lg:flex-none lg:w-50 lg:min-w-[200px] dark:text-gray-200 text-sm font-medium text-left scrollbar-none flex-shrink-0"
	>
		<button
			id="overview"
		...
		>
			...
			<div class=" self-center">{$i18n.t('Budgets')}</div>
		</button>
	</div>
